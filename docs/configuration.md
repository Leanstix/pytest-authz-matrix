...
## Route coverage exclusions

Routes remain coverage targets by default, including the API root generated by DRF's
`DefaultRouter`. Deliberate exceptions must be declared with an audit reason:
...
Each exclusion must define exactly one of `route_name` or `path`. `method` is optional; omitting it
excludes every discovered method matching the selector. Route names may be fully qualified or use
their final unnamespaced component. Path matching uses the same converter and regular-expression
normalization as contracts.

Exclusions are removed from the coverage denominator and are listed with their reasons in terminal
and JSON reports. A route name shared by multiple paths selects every matching route; use a path
selector when only one duplicate-named route should be excluded.
